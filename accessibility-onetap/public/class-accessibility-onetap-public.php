				...
				if ( isset( $settings['icons'], $icon_paths[ $settings['icons'] ] ) ) {
					...
					foreach ( $icons as $icon_value => $icon_image ) {
						if ( $icon_value === $settings['icons'] ) {
							...
							echo '<img class="' . esc_attr( $class_size ) . ' ' . esc_attr( $class_border ) . '" src="' . esc_url( ACCESSIBILITY_ONETAP_PLUGINS_URL . 'assets/images/admin/' . $icon_image ) . '" />';
						}
					}
				} else {
					echo '<img class="design-size2 design-border2" src="' . esc_url( ACCESSIBILITY_ONETAP_PLUGINS_URL . 'assets/images/admin/Original_Logo_Icon.svg' ) . '" />';
				}
				?>
			</div>			
		</section>
